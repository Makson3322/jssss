...
function normalizeObject(input) {
  ...
  o.playing = !!o.playing;
  o.stopwatchRunning = !!o.stopwatchRunning;
  o.stopwatchStart = Number(o.stopwatchStart) || 0;
  o.stopwatchElapsed = Number(o.stopwatchElapsed) || 0;
  if (o.type === 'timer') {
...
// ===== ИСПРАВЛЕННАЯ ФУНКЦИЯ МАСШТАБИРОВАНИЯ =====
function scaleObjectsForResolution(state, nextRes) {
  const prev = state.meta.resolution || DEFAULT_RESOLUTION;
  const rx = nextRes.w / prev.w;
  const ry = nextRes.h / prev.h;
  const liveHeight = prev.h;
  const newLiveHeight = nextRes.h;
  
  Object.values(state.objects).forEach(o => {
    // Масштабируем позицию по X
    o.left = Math.round(o.left * rx);
    
    // Определяем, в какой зоне объект
    const wasInLive = o.top < liveHeight;
    
    if (wasInLive) {
      // Объект в live зоне - масштабируем относительно live
      o.top = Math.round(o.top * ry);
    } else {
      // Объект в staging зоне - масштабируем относительно staging
      const offsetInStaging = o.top - liveHeight;
      o.top = Math.round(newLiveHeight + (offsetInStaging * ry));
    }
    ...
    if (typeof o.fontSize === 'number') {
      o.fontSize = Math.max(8, Math.round(o.fontSize * Math.min(rx, ry)));
    }
    // Масштабируем обводку
    if (typeof o.borderWidth === 'number') {
      o.borderWidth = Math.max(1, Math.round(o.borderWidth * Math.min(rx, ry)));
    }
    // Масштабируем радиус
    if (typeof o.radius === 'number') {
      o.radius = Math.max(2, Math.round(o.radius * Math.min(rx, ry)));
    }
  });
}
